...
@Component({
  ...
  standalone: true,
  imports: [ButtonComponent, VisualizingToolComponent, FormsModule, NgIf],
  template: `
      ...
      <div class="container">
        ...
        <div class="toolContainer">
          <app-visualizing-tool
            [inputUser]="userAsSig()"
            [usingChildData]="usingChildOptions()"
          >
          </app-visualizing-tool>
        </div>
...
export class DataVisualizerComponent {
  ...
  testUser = this.usingCurrentUser()
    ? UserDataHandlerService.Selection
    : new User(this.selectedId, 'Bob');
  ...
  addToSelection() {
    this.uDataService.addUserToLocal(
      (this.testUser = new User(this.selectedId, 'OtherUser')),
    );
